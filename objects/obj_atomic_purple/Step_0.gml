...
if state = atomic_states.normal{
	eye_sprite = spr_atomic_purple_eyes
	...
	eye_sprite = spr_atomic_purple_eyes_attack
	...
	if can_electric{
		can_electric = false
		alarm[1] = electric_cooldown
		electric_eyes = true
		alarm[3] = electric_eyes_frames
		
		instance_create_layer(x,y,"Projectiles",obj_electric)
	}
}
	...
	if state = atomic_states.attack{
		...
		if obj_player.state != states.dead{
			obj_player.state = states.fall{
				...
				instance_create_layer(x,y,layer,obj_part_puple)
				instance_create_layer(x,y,layer,obj_part_puple)
				instance_create_layer(x,y,layer,obj_part_puple)
				instance_create_layer(x,y,layer,obj_part_puple)
				instance_create_layer(x,y,layer,obj_part_purple_eyes)
